...
%
We used a virtual machine with Ubuntu 24.04. Our artifacts are supported in this environment, but due to version-specific requirements, we had to modify some of the source files mentioned in~\ref{s:artifact_abstract}. We used root privileges where appropriate, such as installing specific libraries and tcpdump.
...
\chk{Execution and experimentation}:
%
We execute our tests in either a Mahimahi shell or a CellReplay shell. First, we run Puffer's servers in separate terminals, and in the third terminal, we start an emulated namespace to run our Puppeteer script. The script connects to the servers and runs for 10 minutes. This provides automation for each test run. In the fourth terminal, we start tcpdump and capture the network traffic right before running the script. To further automate this process, TCP packet capture can be added within the script, as well as extracting InfluxDB metrics.

\chk{Metrics and Output}:
%
We explain each metric in detail in Section~\ref{s:background}. To evaluate each model; SSIM index, buffer time, rebuffer time, RTT, and delivery rate are used. These are found in our data set as a CSV file format. In Section~\ref{sec:tcp-analysis}, retransmission is also introduced from our TCP captures, saved in PCAP files.
...
%
We ran 90 tests in total, and each test run takes around 10 minutes. This means that to conduct all 90 tests, for six ABR models in all environments, it takes around 15 hours. If we were to include extracting and processing the dataset, it would take another two weeks to complete experimentation.
