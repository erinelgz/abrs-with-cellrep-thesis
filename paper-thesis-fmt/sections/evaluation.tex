...
In this section, we evaluate six ABR algorithms: \textit{BOLA}, \textit{Gelato}, \textit{Linear BBA}, \textit{MPC}, \textit{Pensieve}, and \textit{TTP}, across various network conditions and different emulators. We report both application-layer metrics and transport-layer metrics, and show how each ABR behaves under identical conditions.
...
We ran experiments on 4G traces in Mahimahi and observed how ABRs behave under a bandwidth-constrained environment. As demonstrated in Figure~\ref{fig:ssim_rebuffer_4g} and Table~\ref{tab:abr_results_4g}, ABRs show clear differences between video quality and smoothness. TTP achieves the highest average SSIM (around 0.9753) and a middle-ground rebuffering time of around 7.82 seconds. 

Pensieve has the lowest rebuffering time (around 6.20 s), but fails to deliver quality as good as other ABRs (with SSIM around 0.9632). This indicates that Pensieve prioritizes smooth playback over quality, by aggressively down-switching quality to smooth playback, averaging a lower SSIM.

MPC and BOLA struggle the most on 4G. Although they deliver a decent quality (SSIM around 0.9747 and 0.9741) they both average a high rebuffering atime round 12.86 s and 13.43 s, respectively.

Finally, Gelato and Linear BBA offer an average performance among all ABRs. They average SSIM values around 0.9747 and 0.9748, and moderate rebuffer times around 6.61 and 6.97, respectively.

In summary, the 4G results show that TTP, Gelato, and Linear BBA have the overall best QoE balance, while Pensieve achieves smoothness at the cost of quality, MPC and BOLA attempting to provide a higher quality, but failing to maintain a smooth playback.
...
With improved network conditions, we used the same 5G traces in both Mahimahi and CellReplay environments to evaluate ABR performance. Using Figure~\ref{fig:mahimahi5g} and Table~\ref{tab:abr_results_mahimahi5g}, our objective is to compare these results to 4G traces, as well as to evaluate how different emulators change the behavior of ABR. All algorithms improved dramatically in both quality and stability. Average rebuffer times drop to around 0.3 to 2.5 seconds, which shows that the capacity of 5G significantly affects playback smoothness for every ABR. Notably, BOLA and MPC have averaged around 0.28 to 0.29 seconds of rebuffer time, ranking as the best in rebuffer time under 5G, whereas on 4G they were ranked the worst. Their improvement shows that the stalling was likely due to limited bandwidth. They also serve a higher quality (SSIM around 0.9828 and 0.9831). This indicates that with sufficient bandwidth, BOLA and MPC can achieve smooth and reasonably high-quality playback.
...
Pensieve also serves the second highest quality (SSIM around 0.9833), but stalls more than other algorithms, with around 2.44s of rebuffering. Interestingly, in previous experiments, Pensieve achieved the smoothest playback with around 6.20 seconds of rebuffering, while providing the lowest quality, whereas in 5G it is the other way around, with the second highest quality but the least smooth. This could be due to overshooting, where it ends up rebuffering more than others.

TTP and Linear BBA maintain an average rebuffer time under approximately 0.3 seconds in this environment, with a reasonably high SSIM (above 0.983). These algorithms can be considered as more stable choices in general, due to their balanced performance.

Overall, the results of 5G in the Mahimahi environment show that higher bandwidth levels play a crucial role in delivering high-quality and smooth playback. All algorithms deliver nearly the same top quality, so the real differentiator here is how successful each algorithm is in avoiding minor stalls.
...
We tested the same 5G traces using CellReplay as an emulator and observed similar high-performance trends (Figure~\ref{fig:cellreplay5g} and Table~\ref{tab:abr_results_cellreplay5g}), with a few noticeable differences compared to Mahimahi. All ABRs achieved close SSIM values around 0.982-0.984. The increase in video quality and smoothness in playback is negligible but still present. Under 5G conditions, all algorithms are capable of delivering top-quality playback.

Gelato maintains the highest SSIM (around 0.9839) also in CellReplay. It still has the second-highest rebuffering (around 1.82), but slightly lower than Mahimahi. Pensieve continues to stall more than other algorithms. This confirms that Pensieve is less cautious than other algorithms in high-throughput networks, as it often overshoots, which causes it to experience stalls despite the available bandwidth.

On the positive side, BOLA, Linear BBA, and MPC manage to keep their rebuffer times under 0.2 seconds, again slightly lower than the experiment in Mahimahi, with a minor improvement in SSIM range (around 0.9827-0.9832).

TTP also maintains a low rebuffer of around 0.23 seconds. These results confirm that most ABRs perform similarly good in sufficient bandwidth. Only Gelato and Pensieve suffer from stalls, trying to achieve top quality.

In general, both emulators stay consistent when the same traces are used in terms of video quality and smoothness. But improved results suggest that CellReplay's more advanced design gives algorithms more headroom to operate in, providing smoother quality switches.

\subsection{Emulator Comparison: Mahimahi vs. CellReplay}

A key part of our evaluation includes directly comparing the differences in both emulators, Mahimahi and CellReplay, using the same 5G traces. This allows us to understand the difference between CellReplay and Mahimahi as an emulation tool, since it was built as a slightly advanced version of Mahimahi, trying to overcome some issues that are present in Mahimahi. By inspecting Table~\ref{tab:abr_results_mahimahi5g} and Table~\ref{tab:abr_results_cellreplay5g}, we can make direct comparisons of overall results. The biggest difference is seen in RTT and delivery rate. CellReplay keeps average RTT around 55-60 ms, which is close to real 5G latency. Mahimahi's RTT is much higher compared to CellReplay. It often averages above 140ms. Mahimahi introduces artificial delays with this simplified delay model.

On the other hand, we can also observe a consistent gap between the two emulators in terms of delivery rate. CellReplay offers higher throughput, with average delivery rates reaching up to 5.3 million bytes per second, while Mahimahi serves up to 3.2 million bytes per second. The advanced design of CellReplay reflects the nature of 5G traffic better than Mahimahi. As a result, this gives ABR algorithms more room to operate, which affects their decision, such as higher quality selections, fewer quality switches, and improves overall quality without stalling as much.

Lower latency and higher throughput affect how ABRs behave. While the changes are not very large, we still observe some impact on rebuffering and video quality. When feedback is delayed, ABRs become more cautious, which can result in slightly more rebuffering and slower adaptation. For instance, Pensieve showed a slightly smaller rebuffering gap between Mahimahi and CellReplay. Gelato, on the other hand, improved more noticeably in CellReplay. This shows that all algorithms are influenced by network conditions, but the extent of that impact depends on how each ABR reacts to changes in throughput and delay.
...
These ranking shifts illustrate how the buffer strategies of each algorithm interact with the emulator design. Mahimahi's inflated delay seems to slightly compress buffer occupancy in general, while CellReplay's realistic model allows for subtle differences in how each ABR maintains its buffer under production-grade 5G conditions. Most ABRs maintain similar buffer levels across both emulators. However, the slight variations in buffer occupancy reflect each ABR's balance between aggressively preloading data and reacting to changing network feedback.

In summary, our comparison shows that the choice of emulator matters in some aspects. Algorithms perform more accurately when realistic network dynamics are used. While Mahimahi might introduce artifacts that can affect ABR behavior, CellReplays better control and offers more realistic results.

\subsection{Trace-Level Variability}

Figure~\ref{fig:rtt-overtime-cellreplay} and ~\ref{fig:rtt-overtime-mahimahi} show RTT differences under different traces (stationary vs. driving) for both emulators. This section focuses on how RTT and Delivery Rate change across traces and networks.

In stationary 5G scenarios, both emulators maintain a lower and more stable RTT over time. In Stationary-Good conditions, CellReplay preserves tightly around 40-60 milliseconds, which reflects realistic 5G latencies, where Mahimahi on average holds RTTs over 140 milliseconds, with high peaks in some points. Under these conditions, algorithms behave almost similarly in CellReplay, whereas in Mahimahi, some ABRs like MPC and Linear BBA have peaks around 400 milliseconds across the timeline. 
...
Table~\ref{tab:abr_results_4g},~\ref{tab:abr_results_mahimahi5g}, and~\ref{tab:abr_results_cellreplay5g} give the overall mean of each metric and ranks of each ABR across three different experiments. For each ABR, we can conclude several patterns that we observe from these comparisons:

\begin{itemize}
    \item \textbf{Gelato} consistently delivers the best SSIM overall. In 5G scenarios, it is the leading algorithm, and in 4G scenarios, it is the second-best in terms of quality. As much as it prioritizes video quality, it sacrifices some overall smoothness most of the time.
    
    \item \textbf{Pensieve} avoids rebuffering more than others when it comes to challenging network conditions. However, in both 5G scenarios, Pensieve performed worse in rebuffering. This indicates that pensieve is sensitive to trace variability. When it comes to slower networks, like 4G, it aggressively lowers quality to prevent stalls. On the other hand, it becomes less cautious in faster networks, like 5G.
    ...
    \item \textbf{BOLA} offers the lowest quality in almost every scenario, but has lower rebuffering compared to other algorithms, making it the smoothest ABR in 5G scenarios. In 4G, it stalls more than any other algorithm, being trace-dependent. BOLA prioritizes smoothness over quality compared to other algorithms in faster networks, but fails to give the same performance in slower ones.
\end{itemize}

More importantly, these results demonstrate that no ABR algorithm can be considered universally the best. Depending on network conditions and user needs, a different ABR may be preferred. The optimal choice, therefore, changes depending on the context.
